...
{
    public class CTNhapKhoController : Controller
    {
        public ActionResult Index(int id, string export_excel = null)
        {
            var phieuNhap = new dbQLKhoEntities().PhieuNhaps.ToList().FirstOrDefault(q => q.ID_PhieuNhap == id);
            ViewBag.phieuNhap = phieuNhap;
            var data = new dbQLKhoEntities().LoHangs.ToList().Where(q => q.ID_PhieuNhap == id && q.Status != 0).ToList();
